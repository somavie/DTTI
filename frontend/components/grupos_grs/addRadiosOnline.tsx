...
export default function RegistroDeRadiosOnline() {
  ...
  const turno = useMemo(() => {
    ...
    if (horaAtual >= 6 && horaAtual < 14) return { id: 1, nome: 'Manhã' }
    if (horaAtual >= 14 && horaAtual < 22) return { id: 2, nome: 'Tarde' }
  ...
  const handleSubmit = async (values: { radiosOnline: { [key: string]: number }; qtdsTotais: { [key: string]: number } }) => {
    try {
      const registros = Object.entries(values.radiosOnline).map(([grupoId, qtdsOnline]) => {
        const qtdsTotais = values.qtdsTotais[grupoId]
        return {
          grupo_id: Number(grupoId),
          turno_id: turno.id,
          relatorio_id: 1,
          qtds_online: qtdsOnline,
          qtds_offline: qtdsTotais - qtdsOnline,
          estado: 1,
        }
      })
