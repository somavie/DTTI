...
interface jsPDFWithAutoTable extends jsPDF {
  setLineDashPattern(arg0: number[], arg1: number): jsPDF;
...
function drawGraph(
  ...
  data: { grupo: string; porcentagem: number }[],
  startY: number
) {
  ...
  doc.setFontSize(14);
  doc.text(
    "Porcentagem de Rádios Online por Grupo",
  ...
  if (data.length === 0) {
    doc.setFontSize(12);
    doc.text(
      "Dados não preenchidos para esta data.",
      doc.internal.pageSize.getWidth() / 2,
      startY + 20,
      {
        align: "center",
  ...
  data.forEach((item, index) => {
    ...
    doc.text(`${item.porcentagem}%`, x + barWidth * 0.4, y - 2, {
      align: "center",
    });
...
export default async function gerarPDF(relatorioId: number) {
  ...
  situacoes.forEach((servico, index) => {
    const serviceText = `${servico.nome}: ${
      servico.operacionabilidade === 1 ? "Operacional" : "Não Operacional"
    }`;
