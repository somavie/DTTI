...
import type { Equipamento, Observacao } from "@/helpers/types"
...
function drawGraph(doc: jsPDFWithAutoTable, data: { grupo: string; porcentagem: number }[], startY: number) {
  ...
  if (data.length === 0) {
    doc.setFontSize(12)
    doc.text("Dados não preenchidos para esta data.", doc.internal.pageSize.getWidth() / 2, startY + 20, {
      align: "center",
    })
    return startY + 40
  }
...
async function fetchSituacoes(): Promise<ServicoType[]> {
  try {
...
export default async function gerarPDF(relatorioId: number) {
  ...
  const { equipamentos, observacoes, entrante, cessante, data_criacao } = await buscarDadosRelatorio(relatorioId)
  ...
  doc.text("Direção de Telecomunicação e Tecnologia de Informação", pageWidth / 2, 60, { align: "center" })
  ...
  const dataSeguinte = new Date(new Date(data_criacao).setDate(new Date(data_criacao).getDate() + 1))
  ...
  yPosition += 10 // Add some space before the services section
  doc.setFontSize(12)
  doc.text("Estado dos Serviços:", marginLeft, yPosition)
  yPosition += 10

  situacoes.forEach((servico, index) => {
    const serviceText = `${servico.nome}: ${servico.operacionabilidade === 1 ? "Operacional" : "Não Operacional"}`
    doc.text(serviceText, marginLeft, yPosition)
    yPosition += 10
   ...
   // Add some space before the services section
   doc.setFontSize(12)
   doc.text("Ocorrencias:", marginLeft, yPosition)
   yPosition += 10
  ...
  // Add some space after the services section
  doc.setFontSize(12)
  doc.text("Equipamentos:", marginLeft, yPosition)
  yPosition += 10
  // Definição das colunas e cabeçalhos da tabela
  const headers = ["Meio", "Quantidade", "Estado", "Local"]
  const columns: Array<keyof Equipamento> = ["nome", "quantidade", "status", "localizacao"]
